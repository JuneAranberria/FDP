...
% obtenidos
% EL ANALISIS DE ESTE CÓDIGO ES DE REDUNDANCIAS, EN VEZ DE DE SINERGIAS
...
%E)
%DIR_DATA='C:\Users\June\OneDrive - Mondragon Unibertsitatea\ERASMUS\0_TFG\WORKING\Chibi\CODES\2024_05_28\HOI_analysis_windows\SLEEP_TRANS\';
%F)
DIR_DATA='C:\Users\June\OneDrive - Mondragon Unibertsitatea\ERASMUS\0_TFG\WORKING\Chibi\CODES\2024_05_28\HOI_analysis_windows\ANES_TRANS\';

% Direccion para guardar los resultados
DIR_OUT = 'C:\Users\June\OneDrive - Mondragon Unibertsitatea\ERASMUS\0_TFG\WORKING\Chibi\CODES\2024_05_28\HOI_analysis_windows\MULTIPLET_ANALYSIS\REDUNDANCIES\';
...
for multiplet = 3:7
    ...
    for i = 1:length(archivos)
        ...
        if startsWith(archivos(i).name, 'Otot')
            ...
            data_mult = data(multiplet).index_var_red;
            % Guardar los valores de la o-info de cada multiplet
            valores_mult = data(multiplet).sorted_red;
    ...
    if ~isempty(results)
        ...
        %A)
        %results_nombre = ['Sleeping_comb_', num2str(multiplet), '_regiones_red.mat'];
        %B)
        %results_nombre=['Anes_comb_', num2str(multiplet), '_regiones_red.mat'];
        %C)
        %results_nombre=['AwakeC_comb_', num2str(multiplet), '_regiones_red.mat'];
        %D)
        %results_nombre=['AwakeD_comb_', num2str(multiplet), '_regiones_red.mat'];
        %PROPOFOL
        %results_nombre=['AnesProp_comb_',num2str(multiplet), '_regiones_red.mat'];
        %E) Creo que es mejor guardarlos normal
        %results_nombre=['SleepTrans_comb_', num2str(multiplet), '_regiones_red.mat'];
        %results_nombre=['SleepTrans_comb_', num2str(multiplet), '_regiones_red_values.mat'];
        %F)
        results_nombre=['AnesTrans_comb_', num2str(multiplet), '_regiones_red.mat'];
        %results_nombre=['AnesTrans_comb_', num2str(multiplet), '_regiones_red_values.mat'];
